...
class MAF(nn.Module):
    def __init__(self, n_blocks, input_size, hidden_size, n_hidden, cond_label_size=None, activation='relu', input_order='sequential', batch_norm=True, u_prior=None, x_prior=None):
        super().__init__()
        # base distribution for calculation of log prob under the model
        self.register_buffer('base_dist_mean', torch.zeros(input_size))
        self.register_buffer('base_dist_var', torch.ones(input_size))
    ...
    @property
    def base_dist(self, mean=None, cov=None):
        return D.Normal(self.base_dist_mean, self.base_dist_var)
    ...
    def generate_prior_samples(self, sample_shape):
        x_samples = None
        u_samples = None
        if self.u_prior is not None:
            u_samples = self.u_prior.sample(torch.Size([sample_shape]))
        elif self.x_prior is not None:
            x_samples = self.x_prior.sample(torch.Size([sample_shape]))
            u_samples, _ = self.net(x_samples, None) # To do: conditional...
     
        return u_samples, x_samples
    
    def log_prob(self, x, y=None, mean=None, cov=None):
        u, sum_log_abs_det_jacobians = self.forward(x, y)
        if mean is None:
            return torch.sum(self.base_dist.log_prob(u) + sum_log_abs_det_jacobians, dim=1)
        else:
            assert cov is not None
            tmp_dist = D.MultivariateNormal(mean, cov)
        
            return tmp_dist.log_prob(u) + torch.sum(sum_log_abs_det_jacobians, dim=1)
            
            
    def weighted_ml_params(self, x, weights, y=None, u_samples=None, u_weights=None):
        u, _ = self.forward(x, y)
        if u_samples is not None:
            assert u_weights is not None
            u = torch.cat([u, u_samples], dim=0)
            weights = torch.cat([weights, u_weights], dim=0)
        mean = torch.matmul(weights, u)
        cov = torch.matmul(torch.transpose(u, 0, 1), torch.matmul(torch.diag(weights), u))
        return mean, cov
...
def train(model, dataloader, optimizer, epoch, args):
    ...
    if args.concentration > 0:
        prior_weight_dist = D.Gamma(torch.tensor([args.concentration/args.T]), torch.tensor([1.0]))
    for i, data in enumerate(dataloader):
        ...
        if weights is None:
            ...
            if args.concentration > 0:
                ...
                u_weights = prior_weight_dist.sample(torch.Size([args.T])).squeeze()
            ...
            mean, cov = model.weighted_ml_params(x, weights, y if args.cond_label_size else None, u_samples, u_weights)
            

            # get loss wrt that weighted mean and covariance
            # since the loss is just the log likelihood, we don't need to incorporate the u_samples here...
            # but, if we have x_samples, then we do need to...
            if x_samples is None:
                loss = - (weights * model.log_prob(x, y if args.cond_label_size else None, mean=mean, cov=cov)).mean(0)
                #loss = - (weights * model.log_prob(x, y if args.cond_label_size else None)).mean(0)
            else:
                assert args.cond_label_size is None # fix later
                x = torch.cat([x, x_samples], dim=0)
                weights = torch.cat([weights, u_weights], dim=0)
                loss = - (weights * model.log_prob(x, y if args.cond_label_size else None, mean=mean, cov=cov)).mean(0)
        optimizer.zero_grad()
        loss.backward()
    ...
    if args.cond_label_size is not None:
        if args.test_labels:
            ...
            for x, y, _ in dataloader:
                x = x.view(x.shape[0], -1).to(args.device)
                y = y.to(args.device)
                logprobs.append(model.log_prob(x, y))
...
@torch.no_grad()
def generate(model, dataset_lam, args, step=None, n_row=10, Xmin=3.0, Xmax=6.0):
    model.eval()

    # conditional model
    if args.cond_label_size:
        if args.test_labels is False:
            ...
            Xdist = D.Uniform(torch.tensor([Xmin]), torch.tensor([Xmax])) # fix to be general
