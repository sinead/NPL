...
def fetch_dataloaders(dataset_name, batch_size, device, flip_toy_var_order=False, toy_train_size=500, toy_test_size=100):
    ...
    if dataset_name in ['GAS', 'POWER', 'HEPMASS', 'MINIBOONE', 'BSDS300']:  # use the constructors by MAF authors
    ...
    elif dataset_name in ['MOONS']:  # use own constructors
        ...
        label_size = train_dataset.label_size
        lam = None
    elif dataset_name in ['SYNTH']: # add on GP-like
        train_dataset = load_dataset(dataset_name)(toy_train_size, False)
        test_dataset = load_dataset(dataset_name)(toy_test_size, True)
        input_dims = train_dataset.input_size
        label_size = train_dataset.label_size
        lam = None

    elif dataset_name in ['GAUSSIANMIX']:
        train_dataset = load_dataset(dataset_name)(toy_train_size, None)
        test_dataset = load_dataset(dataset_name)(toy_test_size, train_dataset.base_dist)
        input_dims = train_dataset.input_size
        label_size = train_dataset.label_size
        lam = None
